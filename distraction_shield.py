...
class AppBlocker:
    def __init__(self, root):
        ...
        # TTK Style elements
        bg = "#333a3b"  # Background colour
        fstyle = ttk.Style()
        fstyle.configure("Frame.TFrame", background=bg)
        lstyle = ttk.Style()
        lstyle.configure("Label.TLabel", foreground="white", background=bg)
        ...
        newprocess_lbl = ttk.Label(
            master_frame,
            text="Enter process to block:",
            font=basic_font,
            width=26,
            style="Label.TLabel",
        )
        self.newprocess_inp = ttk.Entry(master_frame, width=30)
        add_btn = ttk.Button(master_frame, text="Add", command=self.add_process)
        remprocess_lbl = ttk.Label(
            master_frame,
            text="Remove process from blocked:",
            font=basic_font,
            width=26,
            style="Label.TLabel",
        )
        self.remprocess_inp = ttk.Entry(master_frame, width=30)
        del_btn = ttk.Button(master_frame, text="Remove", command=self.remove_process)
        empty_btn = ttk.Button(btn_frame, text="Empty List", command=self.clear_list)
        show_btn = ttk.Button(btn_frame, text="View List", command=self.view_list)
        run_btn = ttk.Button(
            btn_frame,
            text="Run Blocker",
            command=lambda: [self.set_true(), self.run_blocker()],
        )
        stop_btn = ttk.Button(btn_frame, text="Stop Blocker", command=self.stop_blocker)
    ...
    def remove_process(self):
        ...
        if len(remove_txt) == 0:
            self.alert_message(
                f"Please enter an application or process that is in the block list."
            )
        else:
    ...
    def run_blocker(self):
        ...
        if app_list == []:
            self.alert_message("Please add an application or process to block.")
